...
const ScheduleDsrModal = ({ open, close, setSchedulemodal }) => {
  ...
  const [selected, setSelected] = useState(
    JSON.parse(localStorage.getItem("dsremail")) || []
  );
  ...
  const [forminputs, setforminputs] = useState(
    JSON.parse(localStorage.getItem("dsrdetails")) || {
      schedulebasis: "",
      weeklydates: {
        mon: false,
        tue: false,
  ...
  const [status, setStatus] = useState(
    JSON.parse(localStorage.getItem("dsrstatus")) || {
      booked: true,
      intransit: true,
      closed: true,
      arrived: true,
  ...
  return (
    <>
      ...
      >
        ...
        // <DialogActions> */}
            <div className="button_propety d-flex mt-4">
              ...
              <button
                onClick={(e) => {
                  return e.preventDefault(), setSchedulemodal(false);
                }}
                style={{
                  padding: "5px 26px",
                  border: "2px solid #0DA3DE",
      ...
      <DsrUnsubscribeModal
        ...
        close={() => setopenUnsubscribe(false)}
        setopenUnsubscribe={setopenUnsubscribe}
      />
    </>
  );
};
